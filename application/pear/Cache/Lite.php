...
#
# プログラムを更新（山陽新聞　羽場）
# 2009/02/22 キャッシュのディレクトリとファイルを作成する際に権限を777に変更(770,790行目)
# プログラム自体を更新する際はこのプログラム部分の追加を必ず行う
#
...
class Cache_Lite
{
        ...
        if ($this->_hashedDirectoryLevel > 0) {
            ...
            for ($i=0 ; $i<$this->_hashedDirectoryLevel ; $i++) {
                ...
                if (!(@is_dir($root))) {
                    ...
                    #
                    # 権限の変更作業　（2009/02/22 羽場）
                    # これをしないと別のサーバからキャッシュを削除できないため
                    #
                    chmod($root, 0777);
    ...
    */
    function _writeAndControl($data)
    {
        ...
        if (is_object($result)) {
            return $result; #ﾂ黴We return the PEAR_Error object
        }
        $dataRead = $this->_read();
